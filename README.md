...
- **Virtual camera background, fully on-device**: blur your real room or replace it with a built-in backdrop or your own imported image, Zoom-style but local. A tiered matting engine picks the best your machine can sustain: GPU segmentation with guided edge refinement on most machines, and a multithreaded CPU path for low-end ones. Temporal smoothing kills edge shimmer, a light-wrap pass settles you into the backdrop, and an auto governor steps quality down before a take ever stutters (Quality control: Auto / Balanced / Lite). A fourth tier built on RobustVideoMatting over WebGPU lives in the code but is currently gated off: onnxruntime-web 1.27's WebGPU path mis-executes the model's recurrent state, so the alpha matte decays mid-take. It comes back when a newer runtime passes the decay probe. Heavy models download lazily and only on machines that use them; nothing ever leaves your machine
...
Teleprompter in the floating deck · dual-mic mixing / separate tracks · auto zoom-on-click / follow-cursor (needs the Electron wrapper, since a web app can't read your cursor over the shared screen) · GIF export · 9:16 vertical mode · keyframe-snapped instant head-trim · optional Electron wrapper (global hotkeys, full system audio). (Camera background blur/replace, and importing your own background images, shipped with the tiered matting engine, issues #9 and #11.)
