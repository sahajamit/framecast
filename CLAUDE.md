...
## Invariant #11 (added with issue #11 — camera background v2, tiered matting)

**Camera matting is tiered and self-governing, behind one contract.** The `MattingEngine` (`compositor/matting/`) owns model choice and mask refinement behind v1's `push()/getMask()` MaskSource surface; `scene.ts` never knows which tier produced the mask. Rules:

- **The high tier is currently GATED OFF** (`tiers.ts HIGH_TIER_ENABLED`): ort-web 1.27's WebGPU EP mis-executes RVM's recurrent path (alpha decays to zero within seconds — the person fades out of recordings; wasm EP is stable on the same input). Re-validate with the pha-decay probe before flipping the flag on a newer onnxruntime-web. `e2e/person-presence.spec.ts` (real-face fixture through the fake webcam) is the regression guard for this entire "person or backdrop missing from the recorded pixels" class — the green-pattern fake camera can never catch it.
- **Quality only ever degrades gracefully**: high (RVM on WebGPU, when enabled) → balanced (MediaPipe GPU) → lite (MediaPipe WASM, threaded via COOP/COEP) → floor (min-cost inference, builtin backdrops render as blur via `effectiveCameraBackground`) → mask off (raw camera). Init failure demotes (watchFailure); sustained overrun demotes (QualityGovernor, ~1.5 s); upshift needs ~20 s headroom and NEVER happens mid-recording. Tier swaps build the new model in the background while the old one keeps serving masks — no gap, no stall (invariants #2/#8 still hold).
- **Heavy-tier assets never touch devices that won't use them.** onnxruntime + the ~15 MB RVM model load via dynamic import only at tier=high with a background active; they are excluded from the PWA precache and runtime-cached instead (`vite.config.ts` globIgnores + runtimeCaching). They are staged by `scripts/fetch-matting-assets.mjs` (predev/prebuild) into `public/ort` + `public/models`, both gitignored.
- **Refinement is per-inference, never per-composited-frame**: temporal EMA + sigmoid shaping (`maskMath.ts`, pure, unit-tested) then the WebGL2 joint-bilateral guided upsample (`refine.ts`); its canvas IS the published mask. Any refine failure falls back to the model-res mask (the v1 look), never to a blank bubble.
- **The preflight preview caps itself at balanced** (`maxTier: 'balanced'`): recordings on WebGPU machines get strictly-equal-or-better edges than the preview showed. This is the one sanctioned, flattering-direction bend of preview == recording.
- **MediaPipe in the ES-module worker needs the `ModuleFactory` shim** (`mediapipe.ts ensureWasmLoaderGlobal`): its classic-script wasm loader can't register via `import()` in a module worker — this silently broke ALL worker-side segmentation in v1 (recordings fell back to the raw camera while the preview segmented). Don't remove the shim, and don't trust "the preview looks fine" as evidence the recording segments: check `?dbg=seg`.
- **COOP/COEP stays on** (netlify.toml + vite server/preview headers): the lite tier's threaded WASM needs `crossOriginIsolated`, and `e2e/camera-background.spec.ts` asserts it. Any new asset must be same-origin or CORP-tagged.
