...
2. **The user-gesture chain.** The preflight "Start" click opens the Document PiP deck (consumes the activation). `getDisplayMedia` is then called from the "Choose screen & start" click *inside* the deck, using `pipWindow.navigator.mediaDevices`. One click cannot do both.
